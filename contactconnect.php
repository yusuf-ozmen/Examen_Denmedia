...
class Contact {
  ...
  // (C) SAVE RESERVATION
  function save ($firstname, $lastname, $subject) {
    ...
    try {
      $this->stmt = $this->pdo->prepare(
        "INSERT INTO `contact` (`c_firstname`, `c_lastname`, `c_subject`) VALUES (?,?,?)"
      );
      $this->stmt->execute([$firstname, $lastname, $subject]);
    } catch (Exception $ex) {
...
define("DB_NAME", "reserveren");
define("DB_CHARSET", "utf8");
define("DB_USER", "root");
define("DB_PASSWORD", "");

// (F) NEW CONTACT OBJECT
